...
// Достаёт чистый ID видео из ссылки (в том числе с параметрами плейлиста)
// и возвращает нормальный watch-URL, либо null, если это не ссылка на видео
function extractCleanYoutubeUrl(input) {
...
client.once('ready', async () => {
    ...
    if (process.env.YOUTUBE_COOKIE) {
        ...
        console.log('🍪 YouTube cookie применён');
    }
...
client.on('messageCreate', async (message) => {
    ...
    if (message.content.startsWith('!play ')) {
        ...
        if (!query) {
            message.reply('Напиши так: `!play название песни` или `!play ссылка на YouTube`');
            return;
        }

        try {
            let url = extractCleanYoutubeUrl(query);
