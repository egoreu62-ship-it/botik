...
client.on('messageCreate', async (message) => {
    ...
    // ---- Заблокированные пользователи (не слушаем команды от них) ----
    const BLOCKED_USERS = [
        // 'ID_пользователя_1',
        // 'ID_пользователя_2',
    ];
    ...
    if (message.content.startsWith('!play ')) {
        ...
        try {
            ...
            const isYoutubeLink = query.startsWith('http') && play.yt_validate(query) === 'video';
            ...
            // Добавляем в очередь
            queue.push({ stream: streamInfo.stream, inputType: streamInfo.type, title: trackTitle, duration: trackDuration });
