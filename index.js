...
client.on('messageCreate', async (message) => {
    ...
    if (message.content === '!skip') {
        ...
        const canSkip = !requesterId || message.author.id === requesterId || whitelist.includes(message.author.id);

        if (!canSkip) {
            message.reply('❌ Скипнуть этот трек может только тот, кто его включил, или человек из белого списка.');
            return;
        }
...
<body>
    ...
    <div class="card">
        <h3 style="margin-top:0">✅ Белый список (может скипать чужие треки)</h3>
